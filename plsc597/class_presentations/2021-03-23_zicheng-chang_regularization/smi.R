...
#import the data. 
smi <- read.csv(file = here("class_presentations",
                            "2021-03-23_zicheng-chang_regularization",
                            "regSMI.csv"))
...
smiUntidy <-
  smiTib %>%
  pivot_longer(-share, names_to = "Variable", values_to = "Value")
...
linear <- lm(share ~ expertise + similarity + trustworthy + attractive +
               interactivity + parasocial + infoq + recep + involve,
             data = smiTib_train)
summary(linear)
